...
Options::Options(int argc, char **argv)
{
    ...
    auto novalidation = [](const std::string value) {
        return std::string("");
    };
    ...
    option_defs[findoption("measure")].checksanity = measure::validatemeasure;
    ...
    // Default values
    set("checkpointdir", "./measuretest.checkpoint");
    set("restart", "false");
    ...
    while (true) {
        ...
        for (unsigned int i=0; i<nopts; ++i) {
            if (opt == option_defs[i].shortflag) {
                ...
                if (option_defs[i].has_argument) {
                    //! @todo might want to hold off overwriting this?
                    option_defs[i].value = std::string(optarg);
                } else { //! @todo is this the only valid way of dealing with no-arg opts?
    ...
    // Nothing elsewhere verified checkpointdir as being OK.
    std::string errmsg = validatedir(option_defs[findoption("checkpointdir")].value);
    if (errmsg.length() != 0) {
